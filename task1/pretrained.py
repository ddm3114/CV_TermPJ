...
from utlis import read_sample,transform,save_dict,load_dict,train_transforms
import json
...
def train(hyperparameters):
    ...
    for epoch in range(epochs):
        ...
        for iter,sample in enumerate(train_dataloader):
            ...
            for im in img_clone:
                img_transformed = train_transforms(im)  # 应用变换
                img_transformed_list.append(img_transformed)  # 将变换后的图像添加到列表中

            # 将变换后的图像重新组合成一个批次张量，并移动到指定设备
            img_clone= torch.stack(img_transformed_list).to(device)
            img_clone.to(device)
            img = torch.cat([img,img_clone], dim=0)
            img = img.to(device)
            ...
            optimizer.zero_grad()
            output = model(img)
            z_i,z_j = torch.chunk(output,2,0)
